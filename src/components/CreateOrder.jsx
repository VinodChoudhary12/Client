          ...
          <CardContent>
            ...
            {/* <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
              <div className="text-center p-4 bg-blue-50 rounded-lg">
                <div className="text-2xl font-bold text-blue-600">{availableSKUs.length}</div>
                <div className="text-sm text-blue-600">Total SKUs</div>
              </div>
              <div className="text-center p-4 bg-green-50 rounded-lg">
                <div className="text-2xl font-bold text-green-600">
                  {availableSKUs.filter(sku => sku.stock > 50).length}
                </div>
                <div className="text-sm text-green-600">In Stock</div>
              </div>
              <div className="text-center p-4 bg-yellow-50 rounded-lg">
                <div className="text-2xl font-bold text-yellow-600">
                  ₹{availableSKUs.length > 0 ? Math.min(...availableSKUs.map(sku => sku.price)).toLocaleString() : 0} - ₹{availableSKUs.length > 0 ? Math.max(...availableSKUs.map(sku => sku.price)).toLocaleString() : 0}
                </div>
                <div className="text-sm text-yellow-600">Price Range</div>
              </div>
            </div> */}
